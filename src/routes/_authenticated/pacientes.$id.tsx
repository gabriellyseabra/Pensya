...
import {
  ArrowLeft, User, FileText, Wallet, Pencil, Save, X, Upload, Plus, Trash2,
  Phone, Mail, MapPin, GraduationCap, Briefcase, Heart, Baby, Activity, Users,
} from "lucide-react";
...
function PacienteDetailPage() {
  ...
  const { data: modalidades } = useQuery({
    ...
    queryFn: async () => ((await supabase.from("modalidades").select("id, nome").eq("ativo", true).order("nome")).data ?? []) as { id: string; nome: string }[],
  });
  const { data: profissionais } = useQuery({
    queryKey: ["profissionais-ativos"],
    queryFn: async () => ((await supabase.from("profissionais_consultorio").select("id, nome").eq("ativo", true).order("nome")).data ?? []) as { id: string; nome: string }[],
  });
  const { data: escolas } = useQuery({
    queryKey: ["escolas-todas"],
    queryFn: async () => ((await supabase.from("escolas").select("id, nome").order("nome")).data ?? []) as { id: string; nome: string }[],
  });
  ...
  useEffect(() => {
    ...
    async function resolve() {
      if (!paciente?.foto_path) { setSignedFoto(null); return; }
      const bucket = paciente.foto_path.startsWith("pacientes/") ? "pacientes-docs" : "cadastro-publico";
      const { data } = await supabase.storage.from(bucket).createSignedUrl(paciente.foto_path, 60 * 60);
  ...
  const salvar = useMutation({
    mutationFn: async () => {
      const allowed = [
        "nome", "data_nascimento", "genero", "cpf", "documento", "telefone", "email",
        "endereco", "escola_id", "escolaridade", "serie_curso", "contato_escola",
        "modalidade_id", "profissional_responsavel_id", "status", "motivo_status",
        "data_inicio", "data_ultima_avaliacao", "data_alta", "autoriza_imagem",
        "queixa_principal", "expectativas", "hipotese_diagnostica", "observacoes",
        "modelo_pagamento", "valor_acordado", "dia_vencimento", "numero_parcelas",
        "foto_url", "foto_path",
      ];
      const payload: any = {};
  ...
  async function handleFoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/")) { toast.error("Envie JPG ou PNG"); return; }
    setUploadingFoto(true);
    const ext = file.name.split(".").pop()?.toLowerCase() ?? "jpg";
    const path = `pacientes/${id}/foto-${Date.now()}.${ext}`;
    const { error: upErr } = await supabase.storage.from("pacientes-docs").upload(path, file, { contentType: file.type, upsert: true });
    if (upErr) { toast.error(upErr.message); setUploadingFoto(false); return; }
    const { data: signed } = await supabase.storage.from("pacientes-docs").createSignedUrl(path, 60 * 60 * 24 * 365);
    const { error: updErr } = await supabase.from("pacientes").update({ foto_url: signed?.signedUrl ?? null, foto_path: path }).eq("id", id);
    if (updErr) { toast.error(updErr.message); setUploadingFoto(false); return; }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={abaAtiva} onValueChange={setAbaAtiva} className="space-y-4">
        ...
        <TabsContent value="administrativo">
          <AdministrativoTab pacienteId={id} activeTab={adminSubTab} onActiveTabChange={setAdminSubTab} />
        </TabsContent>
      </Tabs>
      ...
      <DataDrawer
        ...
        onOpenChange={(v) => {
          setEditOpen(v);
          if (!v) { setForm(paciente); setResps(paciente.responsaveis ?? []); }
        }}
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={() => salvar.mutate()} disabled={salvar.isPending}>
              <Save className="w-4 h-4 mr-2" />{salvar.isPending ? "Salvando..." : "Salvar alterações"}
            </Button>
          </div>
        }
      >
        <Tabs defaultValue="cadastro" className="space-y-4">
          ...
          <TabsContent value="cadastro" className="space-y-4">
            <Section icon={<User className="w-4 h-4 text-brand" />} title="Dados pessoais">
              <FormRow label="Nome"><Input value={form.nome ?? ""} onChange={e => setForm({ ...form, nome: e.target.value })} /></FormRow>
              <div className="grid grid-cols-2 gap-3">
                <FormRow label="Nascimento"><Input type="date" value={form.data_nascimento ?? ""} onChange={e => setForm({ ...form, data_nascimento: e.target.value })} /></FormRow>
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormRow label="CPF"><Input value={form.cpf ?? ""} onChange={e => setForm({ ...form, cpf: e.target.value })} /></FormRow>
                <FormRow label="Documento"><Input value={form.documento ?? ""} onChange={e => setForm({ ...form, documento: e.target.value })} /></FormRow>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <FormRow label="Telefone"><Input value={form.telefone ?? ""} onChange={e => setForm({ ...form, telefone: e.target.value })} /></FormRow>
                <FormRow label="E-mail"><Input value={form.email ?? ""} onChange={e => setForm({ ...form, email: e.target.value })} /></FormRow>
              </div>
              <FormRow label="Endereço completo"><Textarea rows={2} value={form.endereco ?? ""} onChange={e => setForm({ ...form, endereco: e.target.value })} /></FormRow>
            </Section>

            <Section icon={<GraduationCap className="w-4 h-4 text-brand" />} title="Escola">
              <FormRow label="Escola">
                <Select value={form.escola_id ?? "__none"} onValueChange={v => setForm({ ...form, escola_id: v === "__none" ? null : v })}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormRow label="Escolaridade"><Input value={form.escolaridade ?? ""} onChange={e => setForm({ ...form, escolaridade: e.target.value })} /></FormRow>
                <FormRow label="Série / curso"><Input value={form.serie_curso ?? ""} onChange={e => setForm({ ...form, serie_curso: e.target.value })} /></FormRow>
              </div>
              <FormRow label="Contato da escola"><Input value={form.contato_escola ?? ""} onChange={e => setForm({ ...form, contato_escola: e.target.value })} /></FormRow>
            </Section>

            <Section icon={<Briefcase className="w-4 h-4 text-brand" />} title="Atendimento">
              <FormRow label="Modalidade">
                <Select value={form.modalidade_id ?? "__none"} onValueChange={v => setForm({ ...form, modalidade_id: v === "__none" ? null : v })}>
                  <SelectTrigger><SelectValue placeholder="Selecionar" /></SelectTrigger>
                  <SelectContent>
              ...
              <FormRow label="Profissional responsável">
                <Select value={form.profissional_responsavel_id ?? "__none"} onValueChange={v => setForm({ ...form, profissional_responsavel_id: v === "__none" ? null : v })}>
                  <SelectTrigger><SelectValue placeholder="Selecionar" /></SelectTrigger>
                  <SelectContent>
              ...
              <FormRow label="Status">
                <Select value={form.status ?? "ativo"} onValueChange={v => setForm({ ...form, status: v })}>
                  ...
                  <SelectContent>
                    {PACIENTE_STATUS.map(s => <SelectItem key={s} value={s}>{PACIENTE_STATUS_LABEL[s]}</SelectItem>)}
                  </SelectContent>
                </Select>
              </FormRow>
              <FormRow label="Motivo do status"><Input value={form.motivo_status ?? ""} onChange={e => setForm({ ...form, motivo_status: e.target.value })} /></FormRow>
              <div className="grid grid-cols-2 gap-3">
                <FormRow label="Início"><Input type="date" value={form.data_inicio ?? ""} onChange={e => setForm({ ...form, data_inicio: e.target.value })} /></FormRow>
                <FormRow label="Última avaliação"><Input type="date" value={form.data_ultima_avaliacao ?? ""} onChange={e => setForm({ ...form, data_ultima_avaliacao: e.target.value })} /></FormRow>
              </div>
              <FormRow label="Data de alta"><Input type="date" value={form.data_alta ?? ""} onChange={e => setForm({ ...form, data_alta: e.target.value })} /></FormRow>
              <div className="flex items-center gap-2 pt-1">
                <Switch checked={!!form.autoriza_imagem} onCheckedChange={v => setForm({ ...form, autoriza_imagem: v })} />
                <Label className="text-sm">Autoriza uso de imagem</Label>
              </div>
            </Section>

            <Section icon={<Heart className="w-4 h-4 text-brand" />} title="Clínico">
              <FormRow label="Queixa principal"><Textarea rows={3} value={form.queixa_principal ?? ""} onChange={e => setForm({ ...form, queixa_principal: e.target.value })} /></FormRow>
              <FormRow label="Expectativas"><Textarea rows={3} value={form.expectativas ?? ""} onChange={e => setForm({ ...form, expectativas: e.target.value })} /></FormRow>
              <div className="flex items-center gap-2">
                <Switch checked={!!form.hipotese_diagnostica} onCheckedChange={v => setForm({ ...form, hipotese_diagnostica: v })} />
                <Label className="text-sm">Possui hipótese diagnóstica</Label>
              </div>
              <FormRow label="Observações gerais"><Textarea rows={3} value={form.observacoes ?? ""} onChange={e => setForm({ ...form, observacoes: e.target.value })} /></FormRow>
            </Section>

            <Section icon={<Wallet className="w-4 h-4 text-brand" />} title="Financeiro">
              <FormRow label="Modelo de pagamento">
                <Select value={form.modelo_pagamento ?? "__none"} onValueChange={v => setForm({ ...form, modelo_pagamento: v === "__none" ? null : v })}>
                  <SelectTrigger><SelectValue placeholder="Selecionar" /></SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormRow label="Valor acordado (R$)"><Input type="number" step="0.01" value={form.valor_acordado ?? ""} onChange={e => setForm({ ...form, valor_acordado: e.target.value === "" ? null : Number(e.target.value) })} /></FormRow>
                <FormRow label="Dia de vencimento"><Input type="number" min="1" max="31" value={form.dia_vencimento ?? ""} onChange={e => setForm({ ...form, dia_vencimento: e.target.value === "" ? null : Number(e.target.value) })} /></FormRow>
              </div>
              <FormRow label="Nº de parcelas"><Input type="number" min="1" value={form.numero_parcelas ?? ""} onChange={e => setForm({ ...form, numero_parcelas: e.target.value === "" ? null : Number(e.target.value) })} /></FormRow>
            </Section>
          </TabsContent>

          {/* RESPONSÁVEIS */}
          <TabsContent value="responsaveis" className="space-y-3">
            <Button size="sm" variant="outline" onClick={() => setResps([...resps, { nome: "", parentesco: "", telefone: "", principal: resps.length === 0 }])}>
              <Plus className="w-4 h-4 mr-2" />Adicionar responsável
            </Button>
            {resps.length === 0 && (
              <Card className="glass p-6"><p className="text-sm text-muted-foreground">Nenhum responsável cadastrado.</p></Card>
            )}
                ...
                <Card key={r.id ?? `new-${idx}`} className="glass">
                  <CardContent className="pt-5 space-y-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Switch checked={!!r.principal} onCheckedChange={v => {
                          const next = [...resps];
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>Será removido ao salvar.</AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancelar</AlertDialogCancel>
                            <AlertDialogAction onClick={() => setResps(resps.filter((_, i) => i !== idx))}>Remover</AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </div>
                    <FormRow label="Nome"><Input value={r.nome ?? ""} onChange={e => updateResp(idx, "nome", e.target.value, resps, setResps)} /></FormRow>
                    <div className="grid grid-cols-2 gap-3">
                      <FormRow label="Parentesco"><Input value={r.parentesco ?? ""} onChange={e => updateResp(idx, "parentesco", e.target.value, resps, setResps)} /></FormRow>
                      <FormRow label="Idade"><Input type="number" value={r.idade ?? ""} onChange={e => updateResp(idx, "idade", e.target.value, resps, setResps)} /></FormRow>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <FormRow label="Telefone"><Input value={r.telefone ?? ""} onChange={e => updateResp(idx, "telefone", e.target.value, resps, setResps)} /></FormRow>
                      <FormRow label="E-mail"><Input value={r.email ?? ""} onChange={e => updateResp(idx, "email", e.target.value, resps, setResps)} /></FormRow>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <FormRow label="CPF"><Input value={r.documento ?? ""} onChange={e => updateResp(idx, "documento", e.target.value, resps, setResps)} /></FormRow>
                      <FormRow label="Profissão"><Input value={r.profissao ?? ""} onChange={e => updateResp(idx, "profissao", e.target.value, resps, setResps)} /></FormRow>
                    </div>
                    <FormRow label="Estado civil"><Input value={r.estado_civil ?? ""} onChange={e => updateResp(idx, "estado_civil", e.target.value, resps, setResps)} /></FormRow>
                    <div className="flex items-center gap-2">
                      <Switch checked={!!r.deseja_nf} onCheckedChange={v => updateResp(idx, "deseja_nf", v, resps, setResps)} />
                      <Label className="text-sm">Deseja nota fiscal</Label>
                    </div>
                    {r.deseja_nf && (
                      <FormRow label="Dados para NF"><Textarea rows={2} value={r.dados_nf ?? ""} onChange={e => updateResp(idx, "dados_nf", e.target.value, resps, setResps)} /></FormRow>
                    )}
                  </CardContent>
                </Card>
              ))}
          ...
          <TabsContent value="anamnese" className="space-y-4">
            {(() => {
              const secoes = (preAnamnese?.secoes_estruturadas ?? {}) as Record<string, Record<string, any>>;
              const temEstruturada = Object.keys(secoes).length > 0;
              if (temEstruturada) {
                return (
                  <>
                    <p className="text-xs text-muted-foreground">
                      Reflete a Anamnese preenchida em <strong>Avaliação → Anamnese</strong>. Para editar, use aquela aba.
                    </p>
                    <AnamneseEstruturadaView secoes={secoes} resumos={(preAnamnese?.resumos_secao ?? {}) as Record<string, string>} />
                  </>
                );
              }
              if (!preAnamnese) {
                return (
                  <Card className="glass p-6">
                    <p className="text-sm text-muted-foreground">Nenhuma anamnese preenchida ainda.</p>
                  </Card>
                );
              }
              return (
                <div className="grid gap-4">
                  <JsonBlock title="Gestação" icon={<Baby className="w-4 h-4 text-brand" />} data={preAnamnese.gestacao} />
                  <JsonBlock title="Parto" icon={<Baby className="w-4 h-4 text-brand" />} data={preAnamnese.parto} />
                  <JsonBlock title="Saúde e dados antropométricos" icon={<Activity className="w-4 h-4 text-brand" />} data={preAnamnese.saude} />
                  <JsonBlock title="Contexto familiar" icon={<Users className="w-4 h-4 text-brand" />} data={preAnamnese.contexto_familiar} />
                  <JsonBlock title="Tratamentos anteriores" icon={<FileText className="w-4 h-4 text-brand" />} data={preAnamnese.tratamentos_anteriores} />
                  <JsonBlock title="Outros especialistas" icon={<Users className="w-4 h-4 text-brand" />} data={preAnamnese.outros_especialistas} />
                  <JsonBlock title="Exames clínicos" icon={<Activity className="w-4 h-4 text-brand" />} data={preAnamnese.exames_clinicos} />
                </div>
              );
            })()}
          </TabsContent>
...
function updateResp(idx: number, key: string, value: any, resps: Responsavel[], setResps: (r: Responsavel[]) => void) {
  const next = [...resps];
...
function AnamneseEstruturadaView({
  secoes, resumos,
}: { secoes: Record<string, Record<string, any>>; resumos?: Record<string, string> }) {
  ...
  }).filter(Boolean) as { key: string; titulo: string; linhas: { label: string; valor: string }[]; resumo?: string }[];
    ...
    <div className="grid gap-4">
      {blocos.map((b) => (
        <Card key={b.key} className="glass">
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><FileText className="w-4 h-4 text-brand" />{b.titulo}</CardTitle></CardHeader>
          <CardContent className="space-y-2">
            ...
            {b.resumo && <p className="text-xs text-muted-foreground italic border-t border-border/40 pt-2 mt-2">{b.resumo}</p>}
          </CardContent>
        </Card>
      ))}
    </div>
...
function JsonBlock({ title, icon, data }: { title: string; icon: React.ReactNode; data: any }) {
  const entries = data && typeof data === "object" ? Object.entries(data).filter(([, v]) => v !== null && v !== "" && v !== undefined) : [];
  return (
    <Card className="glass">
      <CardHeader><CardTitle className="text-base flex items-center gap-2">{icon}{title}</CardTitle></CardHeader>
      <CardContent className="space-y-2">
        {entries.length === 0 && <p className="text-sm text-muted-foreground/60">Nenhuma informação preenchida.</p>}
        {entries.map(([k, v]) => (
          <div key={k} className="grid grid-cols-[160px_1fr] gap-3 text-sm">
            <span className="text-muted-foreground capitalize">{k.replace(/_/g, " ")}</span>
            <span className="whitespace-pre-wrap">{typeof v === "boolean" ? (v ? "Sim" : "Não") : typeof v === "object" ? JSON.stringify(v) : String(v)}</span>
          </div>
        ))}
      </CardContent>
    </Card>
