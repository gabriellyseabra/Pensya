...
import {
  ...
  ESTRUTURA_AVALIACAO_NAVE,
  svgPerfilCognitivo,
  legendaClassificacaoHTML,
  type PerfilItem,
...
export const gerarRelatorioPaciente = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: pac } = await supabase
      .from("pacientes")
      .select(
        "nome, data_nascimento, genero, escolaridade, serie_curso, queixa_principal, expectativas, hipotese_diagnostica, observacoes, paciente_diagnosticos(diagnostico:diagnosticos(nome)), escola:escolas(nome), modalidade:modalidades(nome)",
      )
    ...
    const { data: clinicaCfg } = await supabase.from("organizacoes").select("nome").maybeSingle();
    const nomeClinicaPrompt = clinicaCfg?.nome ? ` (${clinicaCfg.nome})` : "";

    const inicio =
      data.periodo_inicio ?? new Date(Date.now() - 90 * 86400000).toISOString().slice(0, 10);
    ...
    const [metasRes, sessoesRes, freqRes, testesRes, planosRes] = await Promise.all([
      supabase
        ...
        .select(
          "id, titulo, descricao, dominio_cognitivo, status, prioridade, data_inicio, data_fim_prevista, criterio_alcance",
        )
      ...
      supabase
        .from("avaliacoes")
        .select(
          "data_avaliacao, motivo, sintese, testes_aplicados(data_aplicacao, classificacao, percentil, escore_padrao, teste:testes_catalogo(nome, dominio:dominios_cognitivos(nome)))",
        )
        .eq("paciente_id", data.paciente_id)
      ...
      supabase
        ...
        .select("titulo, objetivo_participacao, descricao, status, data_inicio, data_fim_previsto")
        .eq("paciente_id", data.paciente_id)
        .eq("status", "ativo")
        .limit(3),
    ]);
    ...
    const taxaPresenca = totalSessoes
      ? Math.round(
          (presencas / (presencas + freq.filter((f: any) => f.tipo !== "presente").length || 1)) *
            100,
        )
      : null;
    ...
    const idade = pac.data_nascimento
      ...
      : null;
    const diagnosticos = (pac.paciente_diagnosticos ?? [])
...
${
  metas
    .map((m: any) => {
      ...
      const ultimos = gs
        .slice(-5)
        .map((g: any) => g.nivel_gas_observado)
        .join(", ");
...
${
  sessoes
    ...
    .map(
      (s: any) =>
        `[${s.data_sessao}] (${s.tipo}) avaliação: ${(s.evolucao ?? s.soap_avaliacao ?? "").slice(0, 300)}`,
    )
    .join("\n") || "—"
}
${data.contexto_extra ? `\nCONTEXTO ADICIONAL:\n${data.contexto_extra}` : ""}
...
${baseContexto}`;
      ...
      system = `Você é uma psicopedagoga clínica sênior redigindo um PLANO TERAPÊUTICO FUNCIONAL no modelo CIF + metas funcionais + escala GAS. Toda meta deve responder "o que muda na participação real". As metas NÃO usam estrutura SMART: o título é funcional e direto, e métricas, percentuais e prazos ficam nos campos próprios (baseline e GAS), nunca na frase da meta.${padraoNave}${instrProf}`;
...
4. Metas funcionais (uma tabela por meta: baseline, níveis GAS -2/-1/0/+1/+2)
...
${baseContexto}`;
    ...
    const grafico = perfilCognitivo.length
      ? legendaClassificacaoHTML() + svgPerfilCognitivo(perfilCognitivo)
      : "";
